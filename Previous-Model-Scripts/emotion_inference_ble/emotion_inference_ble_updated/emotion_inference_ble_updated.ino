/* =============================================================================
   ...
   Sampling rate: ~28.5Hz (35ms per sample) — matches 27-30Hz training data.
   Window: 125 samples (~4.4s), Step: 25 samples (~0.88s), 75% overlap.
...
// ---------------------------------------------------------------------------
// Sampling rate — must match training data (~27-30Hz)
...
int    write_idx                    = 0;
int    total_samples                = 0;   // used only to prime the buffer
int    samples_since_last_inference = 0;
bool   buffer_primed                = false;
unsigned long last_sample_ms        = 0;   // timestamp of last accepted packet
...
// ===========================================================================
void loop() {
  // Rate-limit packet acceptance to match training sample rate (~28.5Hz).
  // We read from the serial buffer whenever data arrives, but only push a
  // sample into the inference buffer every SAMPLE_INTERVAL_MS milliseconds.
  ...
  if (read_packet()) {
    if (now - last_sample_ms >= SAMPLE_INTERVAL_MS) {
      ...
      // Run inference every STEP_SIZE accepted samples once buffer is primed
